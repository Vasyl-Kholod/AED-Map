...
describe('testing settings of checkbox and time fields with initial values', () => {
  ...
  beforeEach(() => {
    ...
    [selectFrom, selectUntil] = screen.getAllByRole(
      'button',
      {
        name: '00:00'
      }
  ...
  it('should check the box to select off', () => {
    ...
    userEvent.selectOptions(
      screen.getByRole('option', { name: /05:00/i })
    );
    expect(selectFrom).toHaveTextContent('05:00');
    userEvent.click(selectUntil);
    expect(
      screen.getByRole('option', { name: /03:00/i })
    ).toHaveClass('Mui-disabled');
    expect(
      screen.getByRole('option', { name: /06:00/i })
...
describe('testing set time fields with custom values', () => {
  it('should check operation at different times and block unavailable options', () => {
    ...
    expect(
      screen.getByRole('option', { name: /14:00/i })
    ).toHaveClass('Mui-disabled');
    ...
    expect(
      screen.getByRole('option', { name: /09:00/i })
    ).not.toHaveClass('Mui-disabled');
  });
